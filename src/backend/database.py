...
def init_database():
    ...
    if announcements_collection.count_documents({}) == 0:
        # Create an example announcement that expires in 30 days
        now = datetime.utcnow()
        example = {
            "message": "Welcome back! Activity registration is open — sign up now.",
            # optional start date left out for this example (immediate)
            "start_date": None,
